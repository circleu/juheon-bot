		...
		try:
			...
			if specialCommandsData[command[0]] == 1:
				...
				if matchWords(command[1][-1], ["초", "분"]):
					...
					return
			elif specialCommandsData[command[0]] == 9:
				await message.channel.guild.create_text_channel(message.channel.name, category=message.channel.category)
